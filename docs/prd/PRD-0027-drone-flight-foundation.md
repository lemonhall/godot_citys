...
### REQ-0027-012 僚机撞击地面时必须复用正式地面爆炸表现与爆炸音效，而不是静默消失

**动机**：`v57` 已经把机群攻击命令和命中结算做通了，但用户手测明确指出：僚机撞击地面时缺少正式的爆炸环、爆炸球和爆炸音效，视觉上像“撞地后直接没了”，破坏打击反馈的一致性。

**范围**：

- 当僚机 strike 进入命中/爆炸阶段时：
  - 必须播放正式的地面爆炸表现；
  - 必须包含至少一层地面爆炸环与一层爆炸球；
  - 必须播放正式爆炸音效，不得静默结算；
  - 必须保持和当前 `weapon 8` / 导弹链一致的爆炸资产方向，不额外发明第三套完全不同的爆炸风格。
- 僚机爆炸表现必须在真实命中的地表位置呈现，不得明显埋入地下或悬空过高。
- 僚机爆炸表现不得破坏现有：
  - 敌人/建筑/行人/车辆爆炸结算；
  - 长机 camera/input owner；
  - `NO SIGNAL` fallback 口径。
- 本轮不要求：
  - 为每种地表材质做不同爆炸材质；
  - 新做高成本粒子系统；
  - 给僚机 impact 单独切镜头。
...
- 自动化测试至少断言：僚机 strike 结算后，debug/event payload 会记录 `impact_fx_played = true`。
- 自动化测试至少断言：僚机 strike 结算后，debug/event payload 会记录至少一次爆炸音效触发。
- 自动化测试至少断言：僚机 strike 结算后，debug/event payload 会暴露正式爆炸音频资源路径，而不是空字符串。
- 自动化测试至少断言：加入 impact FX/音效后，不会破坏既有 leader-only kamikaze fallback。

### REQ-0027-013 僚机自杀冲锋路径必须呈现 deterministic 的俯冲弧线，并且不同僚机之间要有轻微差异

**动机**：用户手测明确指出，当前僚机 strike 路径“太笔直、太匀速”，像调试线段而不像真实俯冲。目标不是做复杂空气动力学，而是在理想化游戏环境里做出可信的攻击感，并保持 deterministic，可回归。

**范围**：

- 僚机进入 strike 后：
  - 不得继续沿“完美直线 + 恒定速度”冲向地面；
  - 必须表现为一条可见的俯冲弧线；
  - 必须带有非恒定速度变化，至少体现“入场”和“末段俯冲”差异。
- 不同僚机之间：
  - 必须使用 deterministic seed 生成轻微不同的路径参数；
  - 差异只允许是小幅 lateral/arc/speed profile 变化；
  - 不得随机到破坏命中点合同。
- 路径差异必须保持 deterministic：
  - 同一输入条件下，测试重跑结果应一致；
  - 不得使用每次运行都不同的真正随机轨迹。
- 本轮不要求：
  - 完整空气阻力/升力模型；
...
- 自动化测试至少断言：resolved strike event 会暴露非零 `path_seed`。
- 自动化测试至少断言：resolved strike event 会暴露非零的曲线路径偏移指标，而不是 `0`。
- 自动化测试至少断言：resolved strike event 会暴露非零的速度变化区间，而不是严格恒速。
- 自动化测试至少断言：同一轮 area strike 的多架僚机，其 `path_seed` 或曲线路径参数不会全部相同。
...
- 僚机撞击地面时必须播放正式爆炸环/爆炸球/爆炸音效，不得静默消失。
- 僚机 strike 路径必须是 deterministic 的俯冲弧线，且不同僚机之间存在轻微差异。
