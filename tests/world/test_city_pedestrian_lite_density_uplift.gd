...
func _run() -> void:
	...
	if not T.require_true(self, int(warm_snapshot.get("tier1_count", 0)) >= M10_WARM_TIER1_MIN, "M10 lite density uplift warm traversal must keep tier1_count at or above the rebalanced target of %d" % M10_WARM_TIER1_MIN):
		return
	...
	if not T.require_true(self, int(first_visit_snapshot.get("tier1_count", 0)) >= M10_FIRST_VISIT_TIER1_MIN, "M10 lite density uplift first-visit traversal must keep tier1_count at or above the rebalanced target of %d" % M10_FIRST_VISIT_TIER1_MIN):
		return
