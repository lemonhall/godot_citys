...
const MIN_VISUAL_HEIGHT_M := 2.5
const MIN_VISUAL_FOOTPRINT_M := 4.0
...
func _run() -> void:
	...
	if scene == null or not (scene is PackedScene):
		T.fail_and_quit(self, "Missing CityPrototype.tscn for fountain landmark visual envelope")
		return
	...
	if not T.require_true(self, player != null and player.has_method("teleport_to_world_position"), "Fountain visual envelope test requires Player teleport API"):
		return
	if not T.require_true(self, chunk_renderer != null and chunk_renderer.has_method("get_chunk_scene"), "Fountain visual envelope test requires chunk renderer lookup"):
		return
	...
	for _frame in range(180):
		...
		if chunk_scene == null or not chunk_scene.has_method("find_scene_landmark_node"):
			continue
		mounted_landmark = chunk_scene.find_scene_landmark_node(FOUNTAIN_LANDMARK_ID) as Node3D
		if mounted_landmark != null:
			break
	if not T.require_true(self, mounted_landmark != null, "Fountain visual envelope test must mount the landmark in chunk_129_142"):
		return

	var visual_extents := _collect_visual_extents(mounted_landmark)
	if not T.require_true(self, int(visual_extents.get("visual_count", 0)) > 0, "Fountain landmark must contribute at least one visible VisualInstance3D"):
		return
	...
	if not T.require_true(self, height_m >= MIN_VISUAL_HEIGHT_M, "Fountain landmark must be visibly tall enough in-world"):
		return
	if not T.require_true(self, maxf(footprint_x_m, footprint_z_m) >= MIN_VISUAL_FOOTPRINT_M, "Fountain landmark must have a visibly readable footprint in-world"):
		return
	if not T.require_true(self, absf(bottom_y - FOUNTAIN_WORLD_POSITION.y) <= MAX_BOTTOM_OFFSET_M, "Fountain landmark visual bottom must stay close to ground level"):
		return
