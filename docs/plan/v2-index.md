...
| REQ-0001-006 | `v2-chunk-streaming.md`, `v2-rendering-lod.md` | `tests/world/test_city_debug_overlay.gd` | `--script res://tests/e2e/test_city_large_world_e2e.gd` | 2026-03-11 已具备 chunk/debug 字段；实例统计待 M3 | in progress |
...
- chunk streaming 当前只驱动生命周期与 debug，尚未接管可见 chunk 渲染实体。
- v1 无 MultiMesh/HLOD/occlusion 的分块约束。
- v1 无分块导航，也无跨多 chunk 的 E2E travel 证据。
- 当前 debug 护栏已有 `current_chunk_id`、`active_chunk_count` 和 streaming 耗时，但缺少实例统计与渲染档位统计。
