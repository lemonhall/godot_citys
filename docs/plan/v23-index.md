...
| M1 landmark placement + map pin | 音乐公路 registry/manifest/scene、full-map 起点 pin | 音乐公路可作为 scene_landmark mount，full map 出现 `music_road` 起点 icon，minimap 不泄漏 | `tests/world/test_city_music_road_manifest_contract.gd`、`tests/e2e/test_city_music_road_full_map_flow.gd` | todo |
| M2 visual road cue | 独立 authored 直线道路、钢琴键视觉、ground alignment | 驾驶视角下可读出钢琴道路语义，visual envelope 贴地且可驾驶 | `tests/world/test_city_music_road_visual_envelope.gd` | todo |
| M3 song trigger runtime | definition contract、entry gate、speed window、`诀别书` 顺序触发 | 目标速度窗口内完整 run 触发正式 note sequence 并标记 success；异常速度/逆向/步行不误判 | `tests/world/test_city_music_road_definition_contract.gd`、`tests/world/test_city_music_road_runtime_sequence_contract.gd`、`tests/world/test_city_music_road_speed_window_contract.gd`、`tests/e2e/test_city_music_road_drive_song_flow.gd` | todo |
| M4 verification | landmark / map / driving / profiling 回归 | 受影响主链不回退，profiling 三件套产出 fresh evidence | `tests/world/test_city_chunk_setup_profile_breakdown.gd`、`tests/e2e/test_city_runtime_performance_profile.gd`、`tests/e2e/test_city_first_visit_performance_profile.gd` | todo |
...
| REQ-0013-003 | `v23-music-road-landmark.md` | `tests/world/test_city_music_road_visual_envelope.gd` | `--script res://tests/e2e/test_city_music_road_drive_song_flow.gd` | — | todo |
| REQ-0013-004 | `v23-music-road-landmark.md` | `tests/world/test_city_music_road_runtime_sequence_contract.gd`、`tests/world/test_city_music_road_speed_window_contract.gd` | `--script res://tests/e2e/test_city_music_road_drive_song_flow.gd` | — | todo |
