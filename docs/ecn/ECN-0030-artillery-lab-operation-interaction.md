...
`M777HowitzerLab` 当前把 `J/L/I/K` 直接暴露成全局热键，玩家一进场就能调炮。这与主世界已有的近距交互口径不一致，也会让未来接入主世界时出现“人在远处走路却还在转炮”的全局输入污染问题。后续实测又发现两个具体缺口：进入操炮态后 HUD 没有持续显示 `J/L`、`I/K` 的提示；同时操炮态直接沿用了 `5m` 进入半径来做自动退出，导致玩家稍微走动就容易丢失操炮权。
...
- 玩家按下 `E` 后进入操炮态；再次按下 `E` 可手动退出。
- 进入操炮态后，HUD 必须持续显示 `按 E 退出操炮  J/L 方位  I/K 高低  R 复位`，不能把玩家留在无提示状态。
- 只有在操炮态激活时，`J/L/I/K` 才能驱动 howitzer yaw / pitch。
- 操炮态必须采用双半径：`5m` 进入、约 `20m` 才自动脱离，避免轻微走位导致误脱离。
