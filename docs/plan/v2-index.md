...
| M2 Chunk Streaming | 玩家驱动的 `5x5` 活跃窗口、后台准备、主线程挂载 | 自动 travel 过程中活跃 chunk 数始终 `<= 25`，chunk 生命周期无重复/漏卸载 | `test_city_chunk_streamer.gd`、`test_city_travel_streaming_flow.gd` | done |
| M3 渲染降级 | chunk-local MultiMesh、HLOD、occluder | 至少一种重复资产走 `MultiMeshInstance3D`；近中远三档表现存在；非整城高模常驻 | `test_city_chunk_renderer.gd`、`test_city_hlod_contract.gd` | done |
| M4 导航与 E2E | chunk nav source、宏观路由、连续穿越验证 | Headless E2E 跨越至少 `2048m` 并输出 chunk 迁移证据；不存在整城单 navmesh 验收路径 | `test_city_navigation_flow.gd`、`test_city_large_world_e2e.gd` | done |
...
| REQ-0001-004 | `v2-rendering-lod.md` | `tests/world/test_city_chunk_renderer.gd` | `--script res://tests/world/test_city_hlod_contract.gd` | 2026-03-11 本地 headless `PASS` | done |
...
| REQ-0001-006 | `v2-chunk-streaming.md`, `v2-rendering-lod.md` | `tests/world/test_city_debug_overlay.gd` | `--script res://tests/e2e/test_city_large_world_e2e.gd` | 2026-03-11 已输出当前 chunk、活跃数、streaming 耗时、实例统计和 LOD 统计 | done |
| REQ-0001-007 | `v2-navigation-e2e.md` | `tests/e2e/test_city_large_world_e2e.gd` | `--script res://tests/e2e/test_city_large_world_e2e.gd` | 2026-03-11 本地 headless `PASS` | done |
...
- v2 已完成大城市底盘所需的世界数据、streaming、占位渲染与导航验证，但 renderer 仍使用占位几何和固定阈值。
- 后续 v3 应聚焦真实美术资产、导航烘焙细化、车辆/行人系统和更复杂的视距策略。
