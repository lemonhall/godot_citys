...
func _run() -> void:
	...
	if not T.require_true(self, absf(height_a - height_a_repeat) <= 0.001, "Terrain sampler must be deterministic"):
		return
	if not T.require_true(self, absf(height_a - height_b) >= 1.0, "Terrain sampler must produce visible elevation change across the city"):
		return
	...
	if not T.require_true(self, chunk_scene.has_method("get_terrain_relief_m"), "Chunk scene must expose get_terrain_relief_m()"):
		return
	if not T.require_true(self, float(chunk_scene.get_terrain_relief_m()) >= 1.0, "Chunk ground must contain visible terrain relief, not a pure flat plane"):
		return
