...
func _run() -> void:
	...
	if not T.require_true(self, not mission_contract.is_empty(), "Requesting a map artillery fire mission must return a formal mission contract instead of silently mutating hidden state"):
		return
	for required_key in ["mission_id", "target_world_position", "battery_snapshot", "solution_state"]:
	...
	if not T.require_true(self, bool(solution_state.get("solved", false)), "In-range artillery fire missions must immediately expose a solved bearing/pitch solution"):
		return
	if not T.require_true(self, float(solution_state.get("horizontal_distance_m", 0.0)) >= 1500.0, "Artillery fire-mission solution must expose the formal gameplay horizontal distance to the target"):
		return
	if not T.require_true(self, float(solution_state.get("pitch_deg", -1.0)) >= 0.0, "Artillery fire-mission solution must preserve the howitzer pitch contract instead of returning a private negative angle"):
		return
