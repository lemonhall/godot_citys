...
const REGISTRY_SCHEMA_VERSION := "v26-scene-minigame-venue-registry-1"
...
func get_entry(venue_id: String) -> Dictionary:
	if venue_id == "":
		return {}
	return (_entries.get(venue_id, {}) as Dictionary).duplicate(true)
...
func save_entry(entry: Dictionary) -> Dictionary:
	var venue_id := str(entry.get("venue_id", ""))
	if venue_id == "":
		return {
			"success": false,
			"error": "missing_venue_id",
		}
	_entries[venue_id] = entry.duplicate(true)
