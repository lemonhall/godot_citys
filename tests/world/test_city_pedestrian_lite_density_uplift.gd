...
func _run() -> void:
	...
	if not T.require_true(self, int(warm_snapshot.get("tier1_count", 0)) >= CURRENT_LITE_WARM_TIER1_MIN, "Current lite density uplift warm traversal must keep tier1_count at or above the frozen target of %d" % CURRENT_LITE_WARM_TIER1_MIN):
		return
	...
	if not T.require_true(self, int(first_visit_snapshot.get("tier1_count", 0)) >= CURRENT_LITE_FIRST_VISIT_TIER1_MIN, "Current lite density uplift first-visit traversal must keep tier1_count at or above the frozen target of %d" % CURRENT_LITE_FIRST_VISIT_TIER1_MIN):
		return
...
func _compact_snapshot(snapshot: Dictionary) -> Dictionary:
	return {
		...
		"active_page_count": int(snapshot.get("active_page_count", 0)),
		"active_state_count": int(snapshot.get("active_state_count", 0)),
		"duplicate_page_load_count": int(snapshot.get("duplicate_page_load_count", 0)),
		"tier1_budget": int(snapshot.get("tier1_budget", 0)),
		"tier1_count": int(snapshot.get("tier1_count", 0)),
		"tier2_budget": int(snapshot.get("tier2_budget", 0)),
		"tier2_count": int(snapshot.get("tier2_count", 0)),
		"tier3_budget": int(snapshot.get("tier3_budget", 0)),
