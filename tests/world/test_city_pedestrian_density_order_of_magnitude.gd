...
func _run() -> void:
	...
	if not T.require_true(self, int(warm_snapshot.get("tier1_count", 0)) >= CURRENT_LITE_WARM_TIER1_MIN, "Current lite warm traversal must keep tier1_count at or above the frozen world baseline of %d" % CURRENT_LITE_WARM_TIER1_MIN):
		return
	...
	if not T.require_true(self, int(first_visit_snapshot.get("tier1_count", 0)) >= CURRENT_LITE_FIRST_VISIT_TIER1_MIN, "Current lite first-visit traversal must keep tier1_count at or above the frozen world baseline of %d" % CURRENT_LITE_FIRST_VISIT_TIER1_MIN):
		return
