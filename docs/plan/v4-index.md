...
| M5 路面连续性修复 | 修复 shared page / tile seam 下的 road surface 断裂 | page/tile 坐标投影正确；first tile 不再漂移到 page center；跨 seam 道路保持连续；不回退到旧的密集圆盘过采样 | `test_city_surface_page_mask_alignment.gd`、`test_city_surface_page_tile_seam_continuity.gd`、`test_city_runtime_performance_profile.gd` | done |
...
- [v4-road-surface-continuity.md](./v4-road-surface-continuity.md)
...
| REQ-0001-004 | `v4-road-surface-cache.md`、`v4-road-surface-lod.md`、`v4-surface-page-architecture.md`、`v4-road-surface-continuity.md` | `tests/world/test_city_ground_road_overlay_material.gd`、`tests/world/test_city_road_surface_lod.gd`、`tests/world/test_city_surface_page_contract.gd`、`tests/world/test_city_surface_page_runtime_sharing.gd`、`tests/world/test_city_surface_page_mask_alignment.gd`、`tests/world/test_city_surface_page_tile_seam_continuity.gd` | `--script res://tests/world/test_city_chunk_setup_profile_breakdown.gd` | M4/M5 已落地 shared page runtime 与 seam continuity 修复；同 page 相邻 chunk 共享 texture，且跨 tile seam 连续 | done |
...
- `M5` 已修正 page road mask 的坐标原点，tile0 不再漂到 page center，跨 tile seam 的道路重新连续。
...
- 当前剩余优先级已经从“路面断裂”转为继续压 page 请求/首次访问成本与总体帧时间。
