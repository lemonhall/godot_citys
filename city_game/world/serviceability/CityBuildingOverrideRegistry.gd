...
const REGISTRY_SCHEMA_VERSION := "v16-building-override-registry-1"

var _primary_registry_path := ""
var _load_registry_paths: Array[String] = []
var _entries: Dictionary = {}

func configure(primary_registry_path: String, load_registry_paths: Array[String] = []) -> void:
...
func get_entry(building_id: String) -> Dictionary:
	if building_id == "":
		return {}
	return (_entries.get(building_id, {}) as Dictionary).duplicate(true)
...
func save_entry(entry: Dictionary) -> Dictionary:
	...
	_entries[building_id] = entry.duplicate(true)
