...
func _run() -> void:
	...
	if not T.require_true(self, ascend_from_e_y >= right_position.y + 0.35, "Holding E during active flight must raise the drone altitude"):
		return
	if not T.require_true(self, float(world.get_player_drone_debug_state().get("vertical_velocity_mps", 0.0)) > 0.0, "Holding E during active flight must produce positive vertical speed"):
		return
	...
	if not T.require_true(self, descend_y <= ascend_from_e_y - 0.25, "Holding Q during active flight must lower the drone altitude"):
		return
	if not T.require_true(self, float(world.get_player_drone_debug_state().get("vertical_velocity_mps", 0.0)) < 0.0, "Holding Q during active flight must produce negative vertical speed"):
		return
	...
	if not T.require_true(self, ascend_from_space_y >= descend_y + 0.25, "Holding Space during active flight must share the formal ascent mapping with E"):
		return
