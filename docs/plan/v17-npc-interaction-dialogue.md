...
交付一条正式、可复用的 NPC 近距交互主链：玩家靠近某个可交互 NPC 到 `3m` 内时，HUD 持续显示“可以按下 `E` 键交互”；玩家按下 `E` 后进入正式对话 runtime；咖啡馆服务员作为首个 consumer 会说出“你想喝点什么？”。
...
- 依赖咖啡馆服务员已存在稳定场景锚点与 idle actor 表现。

## Contract Freeze

- `E` 是正式 NPC 交互键，不是 debug-only 临时键。
- 通用 service actor 的最小 contract 冻结为：`actor_id / display_name / interaction_kind / interaction_radius_m / dialogue_id / opening_line`。
...
- 近距候选只允许来自当前已挂载的 service actor group，不允许每帧扫描全城 pedestrian 数据。
...
- 为功能建筑里的可交互 NPC 建立通用 actor contract
...
2. 运行到红
   - 预期失败点必须落在：当前没有正式 service actor contract、没有 interaction prompt runtime、没有 dialogue runtime，而不是测试本身写错。
3. 实现（绿）
   - 新增 `CityServiceActor.gd` 和 `CityNpcInteractionRuntime.gd`。
...
- 如果 dialogue runtime 只做场景特判，不做通用 contract，第二个 NPC 一上来就要推倒重写。
