...
const REGISTRY_SCHEMA_VERSION := "v25-scene-interactive-prop-registry-1"
...
func get_entry(prop_id: String) -> Dictionary:
	if prop_id == "":
		return {}
	return (_entries.get(prop_id, {}) as Dictionary).duplicate(true)
...
func save_entry(entry: Dictionary) -> Dictionary:
	var prop_id := str(entry.get("prop_id", ""))
	if prop_id == "":
		return {
			"success": false,
			"error": "missing_prop_id",
		}
	_entries[prop_id] = entry.duplicate(true)
