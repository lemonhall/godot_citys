...
static func _build_fallback_proxy_mesh() -> ArrayMesh:
	...
	torso_mesh.size = Vector3(0.42, 0.64, 0.24)
	surface_tool.append_from(torso_mesh, 0, Transform3D(Basis.IDENTITY, Vector3(0.0, 1.18, 0.0)))
	var head_mesh := BoxMesh.new()
	head_mesh.size = Vector3(0.24, 0.28, 0.24)
	surface_tool.append_from(head_mesh, 0, Transform3D(Basis.IDENTITY, Vector3(0.0, 1.66, 0.0)))
	var pelvis_mesh := BoxMesh.new()
	pelvis_mesh.size = Vector3(0.34, 0.22, 0.22)
	surface_tool.append_from(pelvis_mesh, 0, Transform3D(Basis.IDENTITY, Vector3(0.0, 0.76, 0.0)))
	var arm_mesh := BoxMesh.new()
	arm_mesh.size = Vector3(0.11, 0.62, 0.11)
	var left_arm_basis := Basis.from_euler(Vector3(0.0, 0.0, deg_to_rad(8.0)))
	var right_arm_basis := Basis.from_euler(Vector3(0.0, 0.0, deg_to_rad(-8.0)))
	surface_tool.append_from(arm_mesh, 0, Transform3D(left_arm_basis, Vector3(-0.30, 1.10, 0.0)))
	surface_tool.append_from(arm_mesh, 0, Transform3D(right_arm_basis, Vector3(0.30, 1.10, 0.0)))
	var leg_mesh := BoxMesh.new()
	leg_mesh.size = Vector3(0.14, 0.78, 0.14)
	var left_leg_basis := Basis.from_euler(Vector3(0.0, 0.0, deg_to_rad(2.0)))
	var right_leg_basis := Basis.from_euler(Vector3(0.0, 0.0, deg_to_rad(-2.0)))
	surface_tool.append_from(leg_mesh, 0, Transform3D(left_leg_basis, Vector3(-0.10, 0.39, 0.0)))
	surface_tool.append_from(leg_mesh, 0, Transform3D(right_leg_basis, Vector3(0.10, 0.39, 0.0)))
