...
- [已由 ECN-0013、ECN-0015、ECN-0016、ECN-0020 补充] 对 `REQ-0002-016` 的 active density target，必须与本条红线在同一工作区、同一默认 `lite` 配置下同时成立：world contract warm `tier1_count >= 150`、first-visit `tier1_count >= 220`；fresh isolated pedestrian profile warm `ped_tier1_count >= 150`、first-visit `ped_tier1_count >= 220`；fresh isolated runtime warm `ped_tier1_count >= 150`。不得接受“density 绿但 profile 红”或“profile 绿但 density 红”的分裂状态。
- [已由 ECN-0016 补充] 自动化场景压测至少还要覆盖两类真实手玩链路：`tests/e2e/test_city_pedestrian_high_speed_inspection_performance.gd` 必须证明 `inspection` 高速穿越 crowd 时 `wall_frame_avg_usec <= 16667` 且不会误触发 panic/flee；`tests/e2e/test_city_pedestrian_live_gunshot_performance.gd` 必须证明 live gunshot panic chain 下 `wall_frame_avg_usec <= 16667`，且 sampled mid-ring witness 会进入 `panic/flee`、`>400m` outsider 保持 calm。
...
### REQ-0002-016 Lite 模式下的人流平台冻结（新增，见 ECN-0012；目标已由 ECN-0015、ECN-0016、ECN-0020 重定义）

**动机**：`M7-M10` 的 density uplift 已经把系统从 `M6` 的稀疏基线明显拉起来。2026-03-16 的最新手玩反馈进一步把产品判断写死：当前城市已经“足够像一座有活力的城市”，后续重点不是继续把 pedestrian-only 密度往上硬堆，而是冻结当前已接受的平台，并为车辆系统与其他运行时系统保留预算。因此，这条需求从“继续追更高 density”正式重定义为“把默认 `lite` 的城市活力平台稳定冻结在当前平台基线”。

**范围**：

- 默认 `pedestrian_mode = lite` 的 deterministic spawn / occupancy contract 必须在当前 verified plateau 基础上稳定冻结到 warm `>=150`、first-visit `>=220` 的平台；主战场仍然是 `Tier0 + Tier1` 的低成本存在感，而不是继续先抬 Tier 2 / Tier 3 hard cap
...
- 自动化测试至少断言：默认 `lite` 的 world contract 下，warm traversal `tier1_count >= 150`，first-visit traversal `tier1_count >= 220`。
...
- 自动化 profiling / e2e 至少断言：在同一默认 `lite` 配置下，fresh isolated pedestrian profile 的 warm `ped_tier1_count >= 150`、first-visit `ped_tier1_count >= 220`；fresh isolated runtime warm `ped_tier1_count >= 150`。
- 自动化测试至少断言：M9 仍然不通过继续抬高 `tier2_budget` / `tier3_budget` 作为主要解法；除非另开 ECN，否则其 hard cap 继续维持当前口径。
- fresh isolated `tests/e2e/test_city_pedestrian_performance_profile.gd` 与 `tests/e2e/test_city_runtime_performance_profile.gd` 必须继续 `PASS`，且 `wall_frame_avg_usec <= 16667`。
- [已由 ECN-0013、ECN-0015、ECN-0016 补充] 如现有 runtime 无法同时承载本条与 `REQ-0002-007`，必须升级 crowd runtime 架构；不得通过把 `max_spawn_slots_per_chunk`、`lane_slot_budget` 或等价密度参数回退到 `54 / 60` 级别的旧基线、或只在非真实场景下跑空载 profile 来伪造“性能恢复”。
