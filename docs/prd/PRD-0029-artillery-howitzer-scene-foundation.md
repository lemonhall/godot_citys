...
- [已由 ECN-0030 变更] 在 `lab` 中建立上下文化操炮交互：靠近火炮约 `5m` 时出现 `按 E 操作炮` 提示，只有进入操炮态后才允许 `J/L/I/K` 驱动火炮。
...
7. [已由 ECN-0030 变更] 玩家按下 `E` 后进入操炮态，此时才显示 `J/L` 调整方位、`I/K` 调整高低的控制提示；再次按下 `E` 退出操炮态。
...
- 按下 `E` 后进入操炮态，再次按下 `E` 退出；
- 只有在操炮态激活时，`J/L` 才能控制火炮 yaw，`I/K` 才能控制 pitch；
- 退出操炮态后，这四个键必须立即失效，避免它们变成未来主世界里的全局热键；
...
13. [由 ECN-0030 新增] 自动化测试必须证明：只有当玩家进入火炮 `5m` 交互半径时，HUD 才出现 `按 E 操作炮` 提示；进入操炮态前 `J/L/I/K` 不生效，进入后才生效，再次按 `E` 退出后又立即失效。
