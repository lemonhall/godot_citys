...
2. 自动化测试断言世界尺寸固定为 `7000m x 7000m`，主 chunk 尺寸固定为 `256m x 256m`。
3. 自动化测试断言世界生成 API 在未实例化 `CityPrototype.tscn` 的情况下可返回非空 district / road / block / parcel 数据。
4. 反作弊条款：不得把“把当前 `GeneratedCity` 生成更多方块”作为本计划完成证据。
...
1. 写失败测试（红）
   ...
   - 在 `tests/world/test_city_world_generator.gd` 中断言生成器输出非空 district / road / block / parcel 数据。
2. 跑到红
   - 运行：
     ```powershell
