...
func _run() -> void:
	...
	if not T.require_true(self, config_script != null, "Service building map pin runtime absolute-position regression test requires CityWorldConfig.gd"):
		return
	var config = config_script.new()
	if not T.require_true(self, config != null, "Service building map pin runtime absolute-position regression test must instantiate CityWorldConfig"):
		return

	var cafe_manifest: Dictionary = _load_manifest(entries.get("bld:v15-building-id-1:seed424242:chunk_137_136:003", {}))
	if not T.require_true(self, not cafe_manifest.is_empty(), "Cafe full-map pin regression test requires the cafe manifest fixture"):
		return
	...
	if not T.require_true(self, not gun_shop_manifest.is_empty(), "Gun shop full-map pin regression test requires the gun shop manifest fixture"):
		return
	var expected_gun_shop_position: Variant = _resolve_expected_absolute_world_position(config, gun_shop_manifest)
	if not T.require_true(self, expected_gun_shop_position is Vector3, "Gun shop full-map pin regression test must derive an absolute world position from chunk metadata"):
		return
	if not T.require_true(self, _vector3_near(gun_shop_pin.get("world_position", Vector3.ZERO), expected_gun_shop_position as Vector3), "Gun shop full-map pin must resolve to the building's absolute world position instead of chunk-local export coordinates"):
		return
