...
func _run() -> void:
	...
	if not T.require_true(self, not bool(solution_state.get("solved", false)), "Map artillery fire-mission flow must keep the mission pending after target marking instead of showing solved firing data before the player enters操炮状态"):
		return
	if not T.require_true(self, str(solution_state.get("reason", "")) == "requires_live_howitzer_operation", "Pending map artillery fire-mission state must explain that live howitzer operation is still required before solving firing data"):
		return
	...
	if not T.require_true(self, bool(solution_state.get("solved", false)), "After entering操炮状态, map artillery fire-mission flow must expose solved firing data from the real spawned howitzer"):
		return
