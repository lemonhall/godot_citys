...
- `M4` 已启动首个 browser state slice：`CityPrototype.gd`、`PrototypeHud.gd` 与新的 `CityVehicleRadioBrowser.gd` 已接入最小 browser state contract，并以 `tests/world/test_city_vehicle_radio_browser_state_contract.gd` 冻结 `Browse -> countries root` 入口。
