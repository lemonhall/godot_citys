...
func _run() -> void:
	...
	if scene == null or not (scene is PackedScene):
		T.fail_and_quit(self, "Missing CityPrototype.tscn for player drone squadron strike flow")
		return
	...
	if not T.require_true(self, world.has_method("get_player_drone_debug_state"), "Squadron strike flow requires leader debug-state introspection"):
		return
	if not T.require_true(self, world.has_method("get_player_drone_squadron_debug_state"), "Squadron strike flow requires squadron debug-state introspection"):
		return

	var runtime := world.get_node_or_null("PlayerDroneRuntime") as CharacterBody3D
	if not T.require_true(self, runtime != null and runtime.has_method("get_crosshair_state"), "Squadron strike flow requires the mounted PlayerDroneRuntime crosshair API"):
		return

	await _summon_total_drones(world, 5)
	var summoned_state := world.get_player_drone_squadron_debug_state() as Dictionary
	if not T.require_true(self, int(summoned_state.get("active_total_count", -1)) == 5, "Squadron strike flow requires one leader plus four wingmen before command chaining"):
		return
	...
	if not T.require_true(self, runtime.global_position.distance_to(target_world_position) <= TARGET_MAX_DISTANCE_M, "Squadron strike flow requires a reachable FPV target point"):
		return

	_set_mouse_button(MOUSE_BUTTON_LEFT, true)
	_set_mouse_button(MOUSE_BUTTON_LEFT, false)
	var post_single_state := await _wait_for_active_total_count(world, 4, 360)
	var leader_state := world.get_player_drone_debug_state() as Dictionary
	if not T.require_true(self, int(post_single_state.get("active_total_count", -1)) == 4, "Left click with wingmen available must consume exactly one wingman and leave four drones active"):
		return
	if not T.require_true(self, str(leader_state.get("system_state", "")) == "active" and not bool(leader_state.get("strike_committed", false)), "After single wingman dispatch, the leader must remain the active FPV observer instead of self-destructing"):
		return
	if not T.require_true(self, not bool(leader_state.get("signal_loss_active", false)) and not bool(leader_state.get("no_signal_visible", false)), "After single wingman dispatch, the leader must not show NO SIGNAL"):
		return

	_set_mouse_button(MOUSE_BUTTON_MIDDLE, true)
	_set_mouse_button(MOUSE_BUTTON_MIDDLE, false)
	var post_area_state := await _wait_for_active_total_count(world, 1, 480)
	leader_state = world.get_player_drone_debug_state() as Dictionary
	if not T.require_true(self, int(post_area_state.get("active_total_count", -1)) == 1, "Middle-click area strike must eventually consume the remaining wingmen and leave only the leader active"):
		return
	if not T.require_true(self, str(leader_state.get("system_state", "")) == "active" and str(leader_state.get("camera_owner", "")) == "drone", "During the area strike chain, the leader must continue holding the FPV observation context"):
		return
	if not T.require_true(self, not bool(leader_state.get("signal_loss_active", false)) and not bool(leader_state.get("no_signal_visible", false)), "Area strike consumption of wingmen must still avoid leader NO SIGNAL closeout"):
		return
	...
	leader_state = world.get_player_drone_debug_state() as Dictionary
	if not T.require_true(self, bool(leader_state.get("strike_committed", false)), "Once the squadron is down to the last leader, left click must fall back to the original leader kamikaze chain"):
		return

	var signal_loss_state := await _wait_for_strike_state(world, "signal_loss", 240)
	if not T.require_true(self, bool(signal_loss_state.get("signal_loss_active", false)) and bool(signal_loss_state.get("no_signal_visible", false)), "Last-drone leader fallback must still pass through the formal NO SIGNAL closeout"):
		return
