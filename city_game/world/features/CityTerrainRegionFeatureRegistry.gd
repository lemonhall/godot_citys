...
const REGISTRY_SCHEMA_VERSION := "v38-terrain-region-registry-1"
...
func get_entry(region_id: String) -> Dictionary:
	if region_id == "":
		return {}
	return (_entries.get(region_id, {}) as Dictionary).duplicate(true)
...
func save_entry(entry: Dictionary) -> Dictionary:
	...
	_entries[region_id] = entry.duplicate(true)
...
func _merge_registry_path(resource_path: String) -> void:
	...
	for key_variant in entries.keys():
		...
		if entry.is_empty() or _entries.has(key):
			continue
