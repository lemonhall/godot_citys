...
4. 自动化测试断言场景中存在巡检车，可切换到巡检车模式，且切换后 streaming 护栏仍成立。
5. 反作弊条款：不得使用整城单 navmesh 文件、只检查 instantiate、或“直接瞬移到终点就算通过”来宣称 travel 流程通过。
...
- Create: `city_game/scripts/InspectionCarController.gd`
...
- Create: `tests/e2e/test_city_vehicle_inspection_mode.gd`
...
1. 写失败测试（红）
   ...
   - `test_city_vehicle_inspection_mode.gd` 断言巡检车模式可接管 streaming。
2. 跑到红
...
3. 实现（绿）
   ...
   - 增加开发态巡检车，并允许玩家/巡检车切换控制权。
   - 增加稳定的 `build_runtime_report()` 输出。
4. 跑到绿
   - nav 测试、vehicle inspection mode 与 large world E2E 均输出 `PASS`。
5. 必要重构（仍绿）
...
- 如果没有开发态巡检车，人工验证 streaming / ground / HUD 的效率会很差。
