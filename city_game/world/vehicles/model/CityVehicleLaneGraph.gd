...
var _lane_ids_by_road: Dictionary = {}
var _lane_type_counts: Dictionary = {}
var _intersection_turn_contracts: Dictionary = {}
...
func add_lane(lane_data: Dictionary) -> void:
	...
	var road_id := str(stored.get("road_id", ""))
	if not _lane_ids_by_road.has(road_id):
		_lane_ids_by_road[road_id] = []
	(_lane_ids_by_road[road_id] as Array).append(lane_id)
	_register_lane_in_spatial_index(lane_index, stored.get("bounds", Rect2()))

func add_intersection_turn_contract(contract_data: Dictionary) -> void:
	var stored := contract_data.duplicate(true)
	var intersection_id := str(stored.get("intersection_id", ""))
	if intersection_id == "":
		return
	_intersection_turn_contracts[intersection_id] = stored
...
func get_lane_ids_for_road(road_id: String) -> Array[String]:
	...
	for lane_id_variant in _lane_ids_by_road[road_id]:
		lane_ids.append(str(lane_id_variant))
	lane_ids.sort()
	return lane_ids
...
func get_intersection_turn_contract(intersection_id: String) -> Dictionary:
	if not _intersection_turn_contracts.has(intersection_id):
		return {}
	return (_intersection_turn_contracts[intersection_id] as Dictionary).duplicate(true)
