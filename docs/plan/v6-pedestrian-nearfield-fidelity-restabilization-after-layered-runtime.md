# V6 Pedestrian Nearfield Fidelity Restabilization After Layered Runtime

## Goal

在 `M11` 的 full-stack layered runtime 上，把 `M8/M9` 已经拿下的近景 fidelity 要求重新全部收口，确保真实模型、death visual、inspection mode、violent reaction 不会因为 runtime 彻底分层而回退。
...
- 在 `M11` layered runtime 上重新验证并稳定 `Tier2 + Tier3` 真模型、walk/run/death、inspection mode、violent reaction、death visual
- 确保 nearfield 高保真只作用于 fixed nearfield set，不与高密度 Tier 1 / farfield runtime 共用同一套热路径
- 把 `M8/M9` 的 hand-play 需求重新挂到 `M11` 的 promotion / demotion / death visual / threat routing 上

不做什么：

- 不回退 `M11` 的 full-stack layering 与 first-visit redline 成果
- 不新增 police / wanted / rumor / citywide panic 等大玩法
- 不在本轮引入 ragdoll、持久尸体或完整行为树
- 不通过关闭近景 fidelity 来换取 `M11` / `M12` 的 profile 结果
...
4. 本计划边界明确：只做 nearfield fidelity 在 `M11` layered runtime 上的重回归，不扩玩法边界。

## Acceptance

1. 自动化测试必须证明：`Tier2 + Tier3` 在 `M11` layered runtime 上继续实例化真实 civilian model，而不是回退到 BoxMesh 占位。
2. 自动化测试必须证明：7 个 civilian model 的 live rendered height 继续满足 `>= 3.0m` 与 spread 合同，不因 runtime 分层改造重新出现 `animated_woman.glb` 异常巨人或整体矮人化。
...
4. 自动化测试必须证明：凡是模型提供 `death/dead` clip 的 casualty 路径，都继续保留至少 `0.75s` 的可见 death visual 窗口，不因 tier 切换、page runtime、layer demotion 或 chunk remount 被提前抹掉。
5. 自动化测试必须证明：在通过上述 nearfield fidelity 回归后，`tests/e2e/test_city_first_visit_performance_profile.gd`、`tests/e2e/test_city_pedestrian_performance_profile.gd` 与 `tests/e2e/test_city_runtime_performance_profile.gd` 仍继续 `PASS`。
6. 反作弊条款：不得通过重新降低默认人口、屏蔽 inspection mode、关闭真模型、缩短 death visual、或把 violent threat 广播只保留 direct victim 来宣称 `M12` 完成。
...
1. 写失败测试（红）
   - 先在 `M11` layered runtime 上重跑 `M8/M9` 的 nearfield fidelity 测试，确认哪些点因 runtime 分层改造重新变红。
2. 跑到红
   - 必须让失败原因明确落在新的 layering 接缝上，例如 promotion / demotion、death visual lifecycle、inspection routing、violent-state hold，而不是 unrelated breakage。
3. 实现（绿）
   - 调整 Tier 2 / Tier 3 promotion 与 render hooks。
   - 把 death visual 生命周期重新挂到 layered runtime。
   - 校正 inspection 与 violent threat 路由。
   - 重新校准真实模型与 layered runtime 的 visual scaling / animation entry。
4. 跑到绿
   - nearfield fidelity world tests 与 E2E 全部 PASS。
5. 必要重构（仍绿）
   - 清理临时适配代码，确保 nearfield fidelity 与 `M11` runtime 的边界清晰，不再互相侵蚀。
6. E2E / Profiling
   - fresh isolated 重新运行 `test_city_first_visit_performance_profile.gd`、`test_city_pedestrian_performance_profile.gd` 与 `test_city_runtime_performance_profile.gd`，确认 `M12` 没有重新打穿红线。

## Risks

- 如果 `M11` 没有把 nearfield set 与 farfield / tier1 大人口彻底解耦，`M12` 会再次把高密度拖回主线程热路径。
- 如果 death visual 还绑定 live roster 生命周期，任何新的 layer demotion / snapshot 逻辑都会继续把它吞掉。
- 如果 inspection 与 violent threat 仍共用广播链，`M11` 的分层改造会让这个问题更隐蔽而不是消失。
