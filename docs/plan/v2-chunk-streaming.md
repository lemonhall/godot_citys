...
把城市运行时从“单场景常驻”升级为“玩家或巡检车周围有限窗口活跃”，建立大城市底盘最关键的生命周期约束。
...
2. 自动化 travel / drive 测试中，玩家或巡检车跨越至少 8 个 chunk 时，不能出现重复加载同一 chunk ID、卸载遗漏或空节点引用。
3. debug 输出必须包含 `current_chunk_id`、`active_chunk_count` 和至少一个耗时字段。
4. 反作弊条款：禁止通过锁死玩家/巡检车位置、关闭 chunk 切换或只加载一个 chunk 来满足上限。
...
- 如果 streaming 只跟随玩家、不允许巡检车接管视点，人工大范围验收会很低效。
