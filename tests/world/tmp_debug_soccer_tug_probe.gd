...
func _run() -> void:
	...
	for _i in range(30):
		await physics_frame
		await process_frame
	world.debug_set_soccer_ball_state(SOCCER_WORLD_POSITION + Vector3(0.0, 0.6, 0.0), Vector3.ZERO)
	...
	for frame_idx in range(360):
		...
		if frame_idx % 30 == 0:
			print("FRAME ", frame_idx, " BALL_Z ", ball_local.z, " MAX_Z ", max_abs_z, " TOUCH ", touch_team, " KICKS ", ai_debug.get("kick_count", 0), " SWITCHES ", team_switches)
	print("SUMMARY neutral_frames=", neutral_frames, " max_abs_z=", max_abs_z, " team_switches=", team_switches)
	world.queue_free()
	quit()
