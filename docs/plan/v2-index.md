...
| M3 渲染降级与地表承托 | chunk-local MultiMesh、profile-driven HLOD、occluder、GroundBody、sky/fog | 至少一种重复资产走 `MultiMeshInstance3D`；近中远三档表现存在且轮廓连续；不同 chunk 存在确定性变体；离开中心起始区后仍能落在 streamed chunk 地表上；legacy `Ground` 已移除 | `test_city_chunk_renderer.gd`、`test_city_hlod_contract.gd`、`test_city_chunk_variation.gd`、`test_city_visual_environment.gd`、`test_city_chunk_ground_contract.gd`、`test_city_ground_continuity.gd`、`test_city_skeleton_smoke.gd` | done |
...
- v2 已完成 `70km x 70km` 大城市底盘所需的世界数据、streaming、占位地表/渲染、导航验证、运行时证据和开发态高速巡检模式，但 renderer 仍使用占位几何和固定阈值。
