...
func _run() -> void:
	...
	if not T.require_true(self, intercept_world_position_variant is Vector3, "Missile Command wave contract requires each live enemy track to expose recommended_intercept_world_position"):
		return
	var missiles_before := int((world.get_missile_command_hud_state() as Dictionary).get("selected_silo_missiles_remaining", -1))
	...
	if not T.require_true(self, bool(fire_result.get("success", false)), "Missile Command wave contract must let the current silo fire a formal interceptor toward the requested world point"):
		return
	...
	if not T.require_true(self, int(runtime_state.get("destroyed_enemy_count", 0)) >= destroyed_enemy_count_before + 1, "Missile Command wave contract must increase destroyed_enemy_count after a successful intercept"):
		return
	var missiles_after := int((world.get_missile_command_hud_state() as Dictionary).get("selected_silo_missiles_remaining", -1))
	if not T.require_true(self, missiles_before >= 1 and missiles_after == missiles_before - 1, "Missile Command wave contract must decrement the selected silo missile inventory after each formal fire request"):
		return
